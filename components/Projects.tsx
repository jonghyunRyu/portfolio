      ...
      <div className="max-w-6xl mx-auto px-4 py-16">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <div
              key={project.title}
              className="group bg-slate-50 rounded-3xl border border-slate-200 p-6 hover:shadow-2xl hover:border-slate-300 transition-all hover:-translate-y-2"
            >
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {project.tech.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1 bg-slate-200 text-slate-700 rounded-lg text-sm font-medium"
                  >
                    {tech}
                  </span>
                ))}
